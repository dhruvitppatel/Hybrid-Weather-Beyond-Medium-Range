...
def plot_normalized_symmetric_spectrum(s, title_pre, ofil=None):
    ...
    colors = ['#FFFFFF','#C5C9C7','#C0C0C0','#929591','#ADD8E6','#7BC8F6','#90EE90','#15B01A',\
              '#008000','#FFFF14','#FFA500','#FF0000','#8C000F','#FFC0CB']
    levels = [0.9,1.,1.1,1.2,1.3,1.4,1.5,1.7,1.9,2.15,2.4,2.7,3.]
    img = ax.contourf(kmesh0, vmesh0, z, levels=levels, colors=colors, extend='both') 
    for ii in range(3,6):
    ...
    ax.set_xlim([-15,15])
    ax.set_ylim(fb)
    title = title_pre + 'Normalilzed Symmetric Component'
    ax.set_title(title)
...
def calculate_plot_wheeler_kiladis(startdates,prediction_length,timestep):
    ...
    for startdate in startdates:
        ...
        ds_obs = ds_obs.resample(Timestep='6H').sum()
...
#startdates = [datetime(2003,1,16,0)] #pd.date_range(start='1/16/2003', end='2/13/2007', freq='30D')
