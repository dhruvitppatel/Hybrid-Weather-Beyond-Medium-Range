module mod_io
   ...
   contains 
     ...
     subroutine write_netcdf_4d_multi_2d(model_parameters,grid4d,grid3d,timestep,filename,ocean_model)
       ...
       if(timestep.eq.1) then
           ...
           if(model_parameters%precip_bool) then
             ...
             copy3d(3,:,:) = copy3d(3,:,:) * 0.0393701 * 1000.0!* 39.3701
     ...
     subroutine write_netcdf_2d_non_met_data(var,varname,filename,units,x_dim,y_dim)
       real(kind=dp), intent(in)    :: var(:,:)
       character(len=*), intent(in) :: varname
       character(len=*), intent(in) :: filename
       character(len=*), intent(in) :: units
     ...
     subroutine write_netcdf_2d_non_met_data_timeseries(var,varname,filename,timestep,units,x_dim,t_dim)
       real(kind=dp), intent(in)    :: var(:)
       ...
       character(len=*), intent(in) :: filename
       character(len=*), intent(in) :: units
       character(len=*), intent(in) :: x_dim, t_dim
       ...
       if(timestep == 1) then
          call nc_check(nf90_create(path=filename,cmode=NF90_CLOBBER,ncid=file_id),message='write_netcdf_2d_non_met_data create')
          call nc_check(nf90_def_dim(file_id, x_dim, dimsx, xdim_id),message='write_netcdf_2d_non_met_data nf90_def_dimx'//filename//'_'//x_dim)
          call nc_check(nf90_def_dim(file_id, t_dim, NF90_UNLIMITED, timedim_id),message='write_netcdf_2d_non_met_data nf90_def_dimy'//filename)

          arrdims = (/ xdim_id, timedim_id/)
     ...
     subroutine write_netcdf_1d_non_met_data_real_shared_dim(var,varname,filename,units,x_dim,create_dim)
       ...
       if(create_dim) then
         ...
         call nc_check(nf90_inq_dimid(file_id, x_dim,xdim_id),message='write_netcdf_1d_non_met_data_real nf90_inq_dimid')
       endif
     ...
     subroutine read_era_data_parallel_opened(ncid,model_parameters,mpi_res,grid,era_data,start_time_arg,stride_arg)
      ...
      do i=1, num_of_era_vars-1
         ...
         if(grid%periodicboundary) then
           !Do stuff
           !start4d =  [integer::
           !grid%input_xstart,grid%input_ystart,grid%input_zstart,start_time]
           !count4d =  [integer::
           !xgrid-grid%input_xstart+1,grid%inputychunk,grid%inputzchunk,dim_length(4)/stride]
           ...
           !start4d = [integer::
           !1,grid%input_ystart,grid%input_zstart,start_time]
           !count4d = [integer::
           !grid%input_xend,grid%inputychunk,grid%inputzchunk,dim_length(4)/stride]
           ...
           !grid%input_xstart,grid%input_ystart,grid%input_zstart,start_time]
           !count4d = [integer::
           !grid%inputxchunk,grid%inputychunk,grid%inputzchunk,dim_length(4)/stride]
           start4d = [integer:: grid%input_xstart,grid%input_ystart,1,start_time]
   ...
   subroutine read_full_file_4d(grid4d,grid2d,timestep,filename)
    ...
    subroutine open_netcdf_file_parallel(filename,mpi_res,ncid) 
      use mpi
